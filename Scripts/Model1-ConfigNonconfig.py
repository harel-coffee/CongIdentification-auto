...
config=[]
nonconfig=[]
df = pd.read_csv('/content/Model1.csv')
#print(df.head())
## rename columns
...
## train classifier
   ...
   for i in range(len(classes)):
    ...
    class_names1 = {0: 'Configuration', 1:'NotConfiguration'}
    class_names =np.unique(y_test)
    idx = 1

    explainer1=lime.lime_tabular.LimeTabularExplainer(X_train, class_names=class_names) 
    exp1 = explainer1.explain_instance(X_TEST[idx], classifier.predict_proba, num_features=10)
    display(exp.show_in_notebook(show_table=True, show_all=False))
    print('Document id: %d' % idx)
    print('Predicted class =', class_names[classifier.predict(X_TEST[idx]).reshape(1,-1)[0,0]])
